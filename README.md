...
In the browser, `GitHubDB` is also exposed on `window` for DevTools debugging eg. token encyption.  
...
const db = await GitHubDB.owner({
  ...
  rawBranches: ['main', 'master'],              // optional, branches for raw reads (defaults to [branch])
...
**Token pooling.** When multiple tokens are provided, each request picks one at random.  
If a token hits a rate limit or returns 401, the request is automatically retried with a different token from the pool until all are exhausted.
...
Set `enrollToken: false` to skip this registration (e.g. for read-only deployments).  
...
Call `db.permissions(map)` after initialization to configure access rules. Returns `this` so it can be chained.  
...
When `rawBranches` is configured with more than one branch,  
the library compares `Last-Modified` headers across all listed branches and reads from the one with the most recently updated file.  
This is useful for failover or multi-branch setups.  
...
This allows `listDirectoryRaw` (used when `useRaw: true`) to avoid API calls for directory listings,  
falling back to the API only when the index is missing.  
...
On load, the library fetches `updater.js` from the upstream repo and checks whether a newer version of `github-db.js` exists.  
If an update is available, a summary is logged to the console and a small popup is shown in the browser (unless suppressed).  
...
The checker requires network access to `raw.githubusercontent.com`. It does not block initialization.  
